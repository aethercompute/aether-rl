...
aether-rl only ever hosts the trainable policy. Every other model an algorithm
...
class FrozenModelConfig(ClientConfig):
    ...
    aether-rl never launches or updates these — only the trainable policy is
    ever hosted by aether-rl itself. Frozen models are reachable-but-unmanaged:
    ...
    def require_explicit_endpoint(self):
        if "base_url" not in self.model_fields_set and not self.is_elastic:
            raise ValueError(
                ...
                "hosted; aether-rl only ever hosts the trainable policy."
            )
        return self
