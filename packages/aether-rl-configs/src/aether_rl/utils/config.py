...
def find_package_resource(subdir: str) -> Path | None:
    """Find a directory contributed to the `aether_rl` namespace package by any installed wheel.

    Returns None if `subdir` is not present in any wheel — e.g. on a slim
    `aether-rl-configs`-only install where `aether-rl`'s shipped resources
    (templates, etc.) are absent.
    """
    import aether_rl

    for p in aether_rl.__path__:
