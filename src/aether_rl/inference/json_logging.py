...
(`aether_rl.utils.logger.json_sink`). vLLM uses Python's stdlib
...
class JsonFormatter(logging.Formatter):
    ...
    `aether_rl.utils.logger` so the platform's JsonLogsViewer renders
...
def build_dict_config(level: str = "info") -> dict[str, Any]:
    ...
    return {
        ...
        "formatters": {
            "aether_rl_json": {
                "()": "aether_rl.inference.json_logging.JsonFormatter",
            },
        },
...
def write_logging_config(level: str = "info") -> Path:
    ...
    fd, path_str = tempfile.mkstemp(prefix="aether_rl_logging_", suffix=".json")
    path = Path(path_str)
