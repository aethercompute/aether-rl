"""AetherRL extensions to vLLM's `/inference/v1/generate` handler.
...
1. ``data_parallel_rank`` routing — read from the ``X-data-parallel-rank``
   ...
   inference servers AetherRL runs.
...
from aether_rl.inference.vllm.routed_experts import RoutedExpertsCapture


class AetherRlGenerateResponseChoice(GenerateResponseChoice):
    routed_experts: dict[str, Any] | None = None


class AetherRlGenerateResponse(GenerateResponse):
    choices: list[AetherRlGenerateResponseChoice]
...
class _GenerateRoutedExpertsCapture(RoutedExpertsCapture):
    def post_process(self, response: GenerateResponse) -> AetherRlGenerateResponse:
        choices = [
        ...
        return AetherRlGenerateResponse(
...
def _build_usage(final_res: RequestOutput) -> UsageInfo:
    ...
    # but ``/inference/v1/generate`` is aether-rl internal — the cache-discount
...
class AetherRlServingTokens(ServingTokens):
    ...
    async def serve_tokens(
    ...
    ) -> AetherRlGenerateResponse | ErrorResponse | AsyncGenerator[str, None]:
        ...
        if request.stream:
            # Streaming path: defer to upstream — AetherRL's renderer client
    ...
    async def serve_tokens_full_generator(  # type: ignore[override]
        ...
        elif not isinstance(response, AetherRlGenerateResponse):
            # Upgrade to the aether-rl subclass so the declared ``usage`` field
            # actually surfaces in JSON (the parent class would drop it).
            response = AetherRlGenerateResponse(
                request_id=response.request_id,
                choices=[AetherRlGenerateResponseChoice(**choice.model_dump()) for choice in response.choices],
                prompt_logprobs=response.prompt_logprobs,
                kv_transfer_params=response.kv_transfer_params,
            )
