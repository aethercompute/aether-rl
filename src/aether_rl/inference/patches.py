...
from aether_rl.inference.vllm.padded_input_scrub import monkey_patch_vllm_padded_input_scrub


def apply_shared_vllm_patches():
    """vLLM general plugin: aether-rl patches that must run in every vLLM process.
...
def monkey_patch_online_fp8_parameter_cast():
    ...
    if getattr(original_cast, "_aether_rl_unwraps_parameters", False):
        return
...
def monkey_patch_minimax_m2_think_end_passthrough():
    ...
    aether-rl serves MiniMax-M2 with ``reasoning=minimax_m2_append_think``, which
...
def monkey_patch_strip_routed_experts_from_chat():
    ...
    if getattr(OpenAIServingChat.chat_completion_full_generator, "_aether_rl_strips_routed_experts", False):
        return
    ...
    _patched._aether_rl_strips_routed_experts = True
...
def monkey_patch_minimax_m2_for_lora():
    ...
    Problem 2 — Adapter key naming mismatch:
        AetherRL saves adapter keys using its internal naming convention
