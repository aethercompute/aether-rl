...
from aether_rl.trainer.distributed import DeepEPExpertParallel, MXFP8AllToAllExpertParallel
from aether_rl.trainer.lora import apply_lora_to_model, freeze_all_except_lora_and_specified, strip_lora_from_state_dict
from aether_rl.trainer.models import (
    AutoModelForCausalLMAetherRL,
    PreTrainedModelAetherRL,
    PrimeLmOutput,
    cast_float_and_contiguous,
    get_custom_vlm_cls,
    supports_custom_impl,
)
from aether_rl.trainer.models.glm_moe_dsa.sparse_mla_attention import Indexer
from aether_rl.trainer.models.layers.checkpointing import (
    get_supported_targets,
    set_selective_activation_checkpointing,
    supports_selective_activation_checkpointing,
)
from aether_rl.trainer.models.layers.fp8_linear import replace_linear_with_fp8_blockwise_linear
from aether_rl.trainer.models.layers.lm_head import inject_prime_lm_head
from aether_rl.trainer.models.layers.moe import LatentMoE, MoE, TokenChoiceTopKRouter
from aether_rl.trainer.models.layers.mxfp8_grouped_gemm import apply_mxfp8_moe_grouped_gemm
from aether_rl.trainer.models.layers.mxfp8_linear import replace_linear_with_mxfp8_linear
...
from aether_rl.utils.sequence import get_cu_seqlens_from_position_ids
from aether_rl.utils.utils import format_time
from aether_rl.utils.vlm import get_language_model, get_vision_encoder, is_vlm_architecture


def pre_download_model(model_name: str, revision: str | None = None) -> None:
...
def configure_moe_ep_backend(model: nn.Module, config: ModelConfig) -> None:
    ...
    if backend == "deepep":
        from aether_rl.trainer.distributed.deepep import configure_num_sms
...
def get_model(
    ...
    logger = get_logger()
    logger.info(
        f"Loading model config (name={config.name}, revision={config.revision}, "
        f"attn={config.attn}, trust_remote_code={config.trust_remote_code})"
    )
    ...
    # HF Hub, which hard-crashes under HF_HUB_OFFLINE=1. aether-rl swaps in its own mamba_ssm Triton
    ...
    if config.cp > 1 and config.impl == "auto" and impl_to_use != "custom":
        raise ValueError(
            "Context parallelism with model.impl='auto' requires a supported custom AetherRL implementation, "
            "but this architecture resolved to model.impl='hf'."
        )

    if config.vlm is not None and not (is_vlm_arch and custom_vlm_cls):
        raise ValueError(
            "VLM training requires a registered custom AetherRL VLM implementation; "
    ...
    with device:
        if impl_to_use == "custom" and custom_vlm_cls is not None:
            ...
            match impl_to_use:
                ...
                case "custom":
                    model_cls = AutoModelForCausalLMAetherRL

        load_model_start_time = time.perf_counter()
        # HF VLM models require torch_dtype; custom AetherRL models and text Auto models use dtype
...
def load_dcp_from_hf(model: nn.Module, config: ModelConfig, parallel_dims: ParallelDims):
    ...
    def _init_buffers_post_meta():
        if isinstance(model, PreTrainedModelAetherRL):
    ...
    if not Path(config.name).exists():
        snapshot_path = Path(snapshot_download(repo_id=config.name, repo_type="model", revision=config.revision))
    else:
        logger.info(
    ...
    if isinstance(model, PreTrainedModelAetherRL):
        ...
        if snapshot_is_hf and not snapshot_is_prime and model.is_prime_state_dict(model_keys):
            logger.warning(
                "Found HF weight format in snapshot state dict and AetherRL weight format in model state dict. Trying to auto-convert..."
            )
            snapshot_path = convert_dir / "prime"
            if not snapshot_path.exists() and get_world().is_master:
                logger.debug(
                    f"Converting snapshot state dict to AetherRL format and saving to {snapshot_path} on master rank. This is a one-time operation."
                )
            ...
            logger.warning(
                "Found AetherRL weight format in snapshot state dict and HF weight format in model state dict. Trying to auto-convert..."
            )
    ...
    if not isinstance(model, PreTrainedModelAetherRL) and model.config.tie_word_embeddings:
        model.tie_weights()
    _init_buffers_post_meta()
...
def can_reinit_empty_buffers(model: nn.Module):
    ...
    """
    # Custom AetherRL models handle buffer reinit via init_buffers_post_meta
    if isinstance(model, PreTrainedModelAetherRL):
        return True
