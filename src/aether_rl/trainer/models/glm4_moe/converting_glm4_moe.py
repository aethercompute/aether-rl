...
* Shared experts: HF ``mlp.shared_experts.{gate,down,up}_proj.weight`` map to
  ...
  (:class:`~aether_rl.trainer.models.conversion_ops.SqueezeLeading`,
  backward-only).
...
from aether_rl.trainer.models.conversion_ops import (
    GATE_DOWN_UP,
    ConvOp,
    Drop,
    Rename,
...
def glm_moe_layer_ops(layer_idx: int) -> list[ConvOp]:
    """The HF <-> AetherRL MoE ops for one GLM-style layer.

    Shared by GLM-4 MoE and GLM-MoE-DSA (their HF <-> AetherRL MoE conversion is
