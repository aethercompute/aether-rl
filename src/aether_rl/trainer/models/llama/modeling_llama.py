...
from aether_rl.trainer.models.layers.attn import ATTN_IMPL2CLASS, AttentionConfig
from aether_rl.trainer.models.layers.lm_head import PrimeLmOutput
from aether_rl.trainer.models.layers.mlp import MLP, MLPConfig
from aether_rl.trainer.models.layers.norms import RMSNorm, RMSNormConfig
from aether_rl.trainer.models.layers.rotary_emb import RotaryEmbedding, RotaryEmbeddingConfig
from aether_rl.utils.sequence import get_cu_seqlens_from_seq_lens
...
@auto_docstring
class LlamaPreTrainedModel(PreTrainedModelAetherRL):
    config: LlamaConfig
    ...
    def is_prime_state_dict(cls, state_dict: dict[str, Tensor]) -> bool:
        # Dense models use identical key names in HF and AetherRL format, so we
        # never claim to be in a separate AetherRL format. This disables the
        # auto-conversion path in load_dcp_from_hf and lets DCP load directly.
        return False

    @classmethod
    def convert_to_hf(cls, state_dict: dict[str, Tensor]) -> dict[str, Tensor]:
        """Convert state dict from AetherRL training format to HuggingFace format in-place."""
        return state_dict

    @classmethod
    def convert_to_prime(cls, state_dict: dict[str, Tensor]) -> dict[str, Tensor]:
        """Convert state dict from HuggingFace format to AetherRL training format in-place."""
        return state_dict

    @classmethod
    def convert_layer_to_hf(cls, state_dict: dict[str, Tensor], layer_idx: int) -> dict[str, Tensor]:
        """Convert a single layer's state dict from AetherRL format to HuggingFace format in-place."""
        return state_dict

    @classmethod
    def convert_layer_to_prime(cls, state_dict: dict[str, Tensor], layer_idx: int) -> dict[str, Tensor]:
        """Convert a single layer's state dict from HuggingFace format to AetherRL format in-place."""
        return state_dict
    ...
    def forward(
        ...
        *,
        ...
        seq_lens (`torch.LongTensor` of shape `(num_documents,)`):
            Per-document lengths of the packed row (AetherRL packed-batch contract).
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        cache_position (`torch.LongTensor` of shape `(sequence_length)`, *optional*):
            ...
            compatibility — aether-rl asserts `use_cache is None` since training does not
            perform autoregressive decoding, so this argument is unused.
        labels (`torch.LongTensor` of shape `(batch_size, sequence_length)`, *optional*):
            Labels used by AetherRL's wrapped LM head to optionally compute per-token logprobs/entropy.
