...
class PreTrainedModelAetherRL(PreTrainedModel):
    """
    Base class for all AetherRL models that extends HuggingFace PreTrainedModel.
    ...
    def _can_set_experts_implementation(cls) -> bool:
        """AetherRL models use custom MoE implementations and don't support dynamic experts implementation."""
        return False
    ...
    def _check_and_adjust_attn_implementation(
        ...
        AetherRL custom models dispatch attention through their own ``ATTN_IMPL2CLASS``
    ...
    def get_correct_experts_implementation(self, requested_experts: str | None) -> str:
        """AetherRL models always use eager experts implementation."""
        return "eager"
    ...
    def is_prime_state_dict(cls, state_dict: dict[str, Tensor]) -> bool:
        """
        Check if the state dict is in AetherRL training format.

        Args:
            state_dict: The state dict to check.

        Returns:
            True if the state dict is in AetherRL format, False otherwise.
        """
        raise NotImplementedError(f"is_prime_state_dict is not implemented for {cls.__name__}")

    @classmethod
    def conversion_chain(cls, config) -> list:
        """Declarative operations converting between HF and AetherRL state dicts."""
        return []

    def convert_to_hf(self, state_dict: dict[str, Tensor]) -> dict[str, Tensor]:
        """Convert an AetherRL state dict to HuggingFace format in-place."""
        from aether_rl.trainer.models.conversion_ops import apply_prime_to_hf
    ...
    def convert_to_prime(self, state_dict: dict[str, Tensor]) -> dict[str, Tensor]:
        """Convert a HuggingFace state dict to AetherRL format in-place."""
        from aether_rl.trainer.models.conversion_ops import apply_hf_to_prime
    ...
    def convert_layer_to_hf(self, state_dict: dict[str, Tensor], layer_idx: int) -> dict[str, Tensor]:
        """Convert one layer from AetherRL to HuggingFace format in-place."""
    ...
    def convert_layer_to_prime(self, state_dict: dict[str, Tensor], layer_idx: int) -> dict[str, Tensor]:
        """Convert one layer from HuggingFace to AetherRL format in-place."""
        return self.convert_to_prime(state_dict)

    @classmethod
    def convert_adapter_to_hf(cls, state_dict: dict[str, Tensor]) -> dict[str, Tensor]:
        """
        Convert a LoRA adapter state dict from AetherRL training format to HuggingFace format.
        ...
        whose HF naming differs from AetherRL naming at the submodule level (e.g. NemotronH's
    ...
    def convert_layer_to_vllm_kernel(
        ...
        """
        Convert a single layer's state dict from AetherRL format to vLLM kernel format.

        Args:
            state_dict: Layer weights in AetherRL format.
...
__all__ = ["PreTrainedModelAetherRL"]
