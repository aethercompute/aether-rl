...
Every aether-rl model converts between the HuggingFace checkpoint layout and
the AetherRL training layout. Historically each model hand-wrote a pair of
...
how to go *both* directions:
    ...
    forward  (``hf_to_prime``):  HF checkpoint  ->  AetherRL training layout
    backward (``prime_to_hf``):  AetherRL training layout  ->  HF checkpoint
...
@dataclass
class Rename(ConvOp):
    """Rename a single key. ``hf`` is the HF name, ``prime`` the AetherRL name."""
...
class MapValue(ConvOp):
    ...
    prime: str  # the AetherRL-side key the value lives under (after any rename)
...
class Conditional(ConvOp):
    ...
    *current* state dict (``hf_to_prime`` sees HF keys, ``prime_to_hf`` sees
    AetherRL keys), so the branch chosen on the way back can legitimately differ
