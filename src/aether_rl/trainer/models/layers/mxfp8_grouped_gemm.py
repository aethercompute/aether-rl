...
from aether_rl.trainer.models.layers.moe import GroupedExperts, NonGatedGroupedExperts
...
def _fallback_to_triton_rearrange_for_wide_moes() -> None:
    if getattr(tao_mxfp8_gmm.mx_block_rearrange_2d_M_groups_cuda, "_aether_rl_wide_moe", False):
        return
    ...
    mx_block_rearrange_2d_M_groups._aether_rl_wide_moe = True
...
def _relax_torchao_mxfp8_version_gate() -> None:
    if getattr(cu._get_tensor_cls_for_config, "_aether_rl_relaxed", False):
        return
    ...
    _get_tensor_cls_for_config._aether_rl_relaxed = True
...
def _align_permute_indices_buffer() -> None:
    ...
    """
    if getattr(tt_indices.generate_permute_indices, "_aether_rl_aligned", False):
        return
    ...
    generate_permute_indices._aether_rl_aligned = True
