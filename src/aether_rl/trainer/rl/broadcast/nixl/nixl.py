...
from aether_rl.configs.trainer import NIXLWeightBroadcastConfig
...
from aether_rl.trainer.rl.broadcast.nixl.agent import NixlAgent, make_agent_name, set_ucx_env_defaults
...
from aether_rl.trainer.rl.broadcast.nixl.trainer_tensor_table import (
    TrainerAgent,
    TrainerGroup,
    TrainerShard,
    TrainerTensor,
...
from aether_rl.trainer.utils import get_world
...
class NIXLWeightBroadcast(WeightBroadcast):
    ...
    def initialize_transfer(self, model: nn.Module) -> None:
        if self.initialized:
            return
        model = cast(PreTrainedModelAetherRL, model)
