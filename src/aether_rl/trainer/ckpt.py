...
from aether_rl.configs.trainer import CheckpointConfig, LoRAConfig, WeightCheckpointConfig
from aether_rl.trainer.lora import has_lora_layers, save_lora_config
from aether_rl.trainer.models import PreTrainedModelAetherRL
from aether_rl.trainer.optim import CPUOffloadOptimizer
from aether_rl.trainer.runs import Progress, get_multi_run_manager
...
from aether_rl.utils.utils import get_all_ckpt_steps, get_ckpt_dir, get_step_path, get_weights_dir
...
class WeightCheckpointManager:
    ...
    def save(
        ...
        if isinstance(model, PreTrainedModelAetherRL) and model.is_prime_state_dict(state_dict):
            self.logger.debug("Converting AetherRL format to HF format for weight checkpoint")
            start_time = time.perf_counter()
            model.convert_to_hf(state_dict)
            self.logger.debug(
                f"Converted AetherRL format to HF format in {time.perf_counter() - start_time:.2f} seconds"
            )
        else:
