...
from aether_rl.utils.monitor.base import Monitor, sample_items_for_logging
...
# aether-rl logs many metrics; the default workspace auto-generates a panel per key, which buries the
...
def line_panels(metrics: Sequence[str], regexes: Sequence[str]) -> list[wr.LinePlot]:
    ...
    # "_timestamp"); everything else on "step" (aether-rl's logged training step, not internal "Step").
