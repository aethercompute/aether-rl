...
# Allow runtime override of the aether-rl source itself.
# AETHER_RL_REF can be a git tag, branch, or commit hash. AETHER_RL_REPO
...
# `uv sync --inexact` re-installs aether_rl from the override source.
# Workdir + PATH swap so the chart's `uv run trainer/inference/orchestrator`
# resolves the entrypoints from the override venv.
if [ -n "$AETHER_RL_REF" ]; then
    AETHER_RL_REPO="${AETHER_RL_REPO:-https://github.com/aethercompute/aether-rl.git}"
    ...
    REF_SLUG="${AETHER_RL_REF//\//-}"
    REF_HASH=$(echo -n "${AETHER_RL_REPO}|${AETHER_RL_REF}" | md5sum | cut -c1-12)
    DEST="/tmp/aether-rl-${REF_SLUG}-${REF_HASH}"
    ...
    if [ ! -d "$DEST/.git" ]; then
        echo "[aether-rl] cloning ${AETHER_RL_REPO} for ${AETHER_RL_REF}"
        rm -rf "$DEST"
        git clone --recurse-submodules "$AETHER_RL_REPO" "$DEST"
    fi
    # Always fetch + checkout so mutable refs (branches/tags) pick up new
    # commits between pod restarts. No-op for immutable SHAs.
    echo "[aether-rl] refreshing ${AETHER_RL_REF}"
    git -C "$DEST" fetch --quiet --tags --force origin
    git -C "$DEST" checkout --quiet --force "$AETHER_RL_REF"
    # Fast-forward to upstream tip when AETHER_RL_REF is a branch name.
    # Silently no-ops for SHAs/tags (no `origin/<sha>` exists).
    git -C "$DEST" reset --hard --quiet "origin/${AETHER_RL_REF}" 2>/dev/null || true
...
# AETHER_RL_REF swap so when both are set the install lands in the override
