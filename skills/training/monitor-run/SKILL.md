...
For a disk-full trainer failure, stop the coordinator before changing files. Preserve every checkpoint containing `STABLE` and every published policy. A checkpoint directory for the failed next step that lacks `STABLE` is unpublished partial output and may be removed while stopped. Free enough capacity outside the active run for all remaining mandatory checkpoints plus temporary-write headroom, then restart the coordinator with the same run root and configuration so it resumes from the active policy's stable checkpoint.
