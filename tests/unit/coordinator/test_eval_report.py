...
def test_summarize_eval_groups_by_behavior_policy(tmp_path):
    ...
    payload = ProcessedGroupPayload(
        ...
        rollouts=[],
        evaluation_records=[
            {"ok": True, "rewards": {"accuracy": 1.0}, "metrics": {"exact_format": 1.0}},
            {"ok": True, "rewards": {"accuracy": 0.0}, "metrics": {"exact_format": 0.0}},
            {"ok": False, "rewards": {"accuracy": 1.0}, "metrics": {}},
        ],
    )
    (groups_dir / "group.msgpack").write_bytes(msgspec.msgpack.encode(payload))

    assert summarize_eval(groups_dir, source_id="codeword-eval") == [
        {
            "source_id": "codeword-eval",
            "policy_version": 3,
            "policy_id": "policy-3",
